...
import Gitea from '../implementation';
...
describe('gitea backend', () => {
  ...
  it('throws if configuration does not include repo', () => {
    expect(() => resolveBackend({ backend: {} })).toThrowErrorMatchingInlineSnapshot(
      `"The Gitea backend needs a \\"repo\\" in the backend configuration."`,
    );
  });
  ...
  describe('authenticate', () => {
    it('throws if user does not have access to project', async () => {
      ...
      await expect(
        ...
        `"Your Gitea user account does not have access to this repo."`,
      );
    });
