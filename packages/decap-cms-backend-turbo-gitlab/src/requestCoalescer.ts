 ...
 * concurrently, and each locale that misses the content cache reads the *same*
 * parent-folder listing to resolve its own blob. Measured on the GitHub twin's
 * tester site as two identical tree requests fired in the same tick, 2.7s and
 * 4.4s; the GitLab read path fans out the same way. Through a proxy that
 * charges a fixed ~1.3s preamble per request, the duplicate is pure waste.
 *
 ...
 *
 * NOTE: mirrored in decap-cms-backend-turbo-github, for the same reason
