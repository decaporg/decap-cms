...
const IconWrap = styled.div`
  background-color: ${({ type, theme }) => {
    if (type === TYPE.SUCCESS) return color(theme.color.success['900']).alpha(0.2).string();
    if (type === TYPE.WARNING) return color('#FFB81C').alpha(0.2).string();
    if (type === TYPE.ERROR) return color(theme.color.danger['900']).alpha(0.2).string();
