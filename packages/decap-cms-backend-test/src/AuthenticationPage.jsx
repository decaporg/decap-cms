...
const LoginButton = styled(Button)`
  ...
  ${({ color }) =>
    ...
    &:hover:focus,
    &:focus {
      background-color: ${Color(color).lighten(0.1).string()};
    }
    &:active,
    &:active:focus {
      background-color: ${Color(color).darken(0.1).string()};
    }
  `
      : ``}
`;
...
function AuthenticationPage({ onLogin, inProgress, config, t, handleSubmit }) {
  ...
  function handleSignin(backend, e) {
    e.preventDefault();
