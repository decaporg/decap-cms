...
export default class API {
  ...
  constructor(config: Config) {
    ...
    const [repoParts, originRepoParts] = [
      this.repo.split("/"),
      this.originRepo.split("/"),
    ];
  ...
  user(): Promise<
    { full_name: string; login: string; avatar_url: string; email: string }
  > {
  ...
  urlFor(path: string, options: Options) {
    ...
    if (options.params) {
      for (const key in options.params) {
        params.push(
          `${key}=${encodeURIComponent(options.params[key] as string)}`,
        );
      }
    }
    if (params.length) {
  ...
  async fetchBlobContent({ sha, repoURL, parseText }: BlobArgs) {
    const result: GitGetBlobResponse = await this.request(
  ...
  async listFiles(
    ...
    { type: string; id: string; name: string; path: string; size: number }[]
  > {
    ...
    try {
      ...
      return (
        result.tree
          ...
          .map((file) => {
            const relativePath = hasFolder && file.path.startsWith(`${folder}/`)
              ? file.path.slice(folder.length + 1)
              : file.path;
            return {
              type: file.type,
      ...
      if (err && err.status === 404) {
        console.info(
          "[StaticCMS] This 404 was expected and handled appropriately.",
        );
        return [];
      } else {
        throw err;
    ...
    const items: ChangeFileOperation[] = await Promise.all(
      files.map(async (file) => {
        ...
        try {
          ...
          const newPath = "newPath" in file
            ? (file as DataFile).newPath
            : undefined;
  ...
  async getFileSha(
    path: string,
    { repoURL = this.repoURL, branch = this.branch } = {},
  ) {
  ...
  async getOpenAuthoringPullRequest(
    ...
    const pullRequest =
      pullRequests.filter((pr) => pr.head.sha === data.commit.id)[0];
    if (!pullRequest) {
    ...
    const nonCmsLabels = pullRequest.labels.filter((l) =>
  ...
  async getBranchPullRequest(branchName: string): Promise<ForgejoPullRequest> {
    if (this.useOpenAuthoring) {
      ...
      const result = await this.getOpenAuthoringPullRequest(
        branchName,
        pullRequests,
      );
  ...
  async getDifferences(
    from: string,
    to: string,
  ): Promise<ForgejoCompareResponse> {
    ...
    try {
      return await this.request(
        `${repoURL}/compare/${encodeURIComponent(from)}...${
      ...
      if (this.useOpenAuthoring) {
        ...
        return this.request(
          `${this.originRepoURL}/compare/${encodeURIComponent(from)}...${
            encodeURIComponent(to)
          }`,
        );
      }
  ...
  async setPullRequestStatus(
    ...
    await this.updatePullRequestLabels(pullRequest.number, [
      ...currentLabels,
      label.id,
    ]);
  }
  ...
  async getOpenAuthoringBranches(): Promise<ForgejoBranch[]> {
    const branches: ForgejoBranch[] = await this.requestAllPages(
  ...
  filterOpenAuthoringBranches = async (branch: string) => {
    try {
      ...
      const { state: currentState, merged_at: mergedAt } =
        pullRequest as ForgejoPullRequest;
      if (
        pullRequest.number !== MOCK_PULL_REQUEST && currentState === "closed" &&
        mergedAt
      ) {
  ...
  async listUnpublishedBranches(): Promise<string[]> {
    if (this.useOpenAuthoring) {
      ...
      branches = branchesWithFilter.filter((b) => b.filter).map((b) =>
        b.branch
      );
      return branches;
    }
  ...
  async retrieveUnpublishedEntryData(contentKey: string) {
    ...
    if (this.useOpenAuthoring) {
      ...
      const pullRequests = await this.getPullRequests("all", headRef);
      const openAuthoringResult = await this.getOpenAuthoringPullRequest(
        branch,
        pullRequests,
      );
    ...
    const statusLabel = pullRequest.labels.find((l) =>
      isCMSLabel(l.name, this.cmsLabelPrefix)
    );
    ...
    return {
      ...
      pullRequestAuthor: pullRequest?.user?.login ||
        branchData?.commit?.author?.name || "Unknown",
    };
  }
  ...
  async updateUnpublishedEntryStatus(
    ...
    if (pullRequest.number !== MOCK_PULL_REQUEST) {
      ...
      const diff = await this.getDifferences(
        this.branch,
        await this.getHeadReference(branch),
      );
      const title = diff.commits[0]?.commit?.message ||
        API.DEFAULT_COMMIT_MESSAGE;
  ...
  async publishUnpublishedEntry(collection: string, slug: string) {
    ...
    if (pullRequest.number === MOCK_PULL_REQUEST) {
      throw new APIError(
        "Cannot publish entry without a pull request",
        400,
        API_NAME,
      );
    }
  ...
  async forkExists(): Promise<boolean> {
    try {
      ...
      const forkExists: boolean = repo.fork === true &&
        !!repo.parent &&
