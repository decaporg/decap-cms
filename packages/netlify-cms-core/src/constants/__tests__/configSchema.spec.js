...
import { validateConfig, hasCustomWidgetSchemas } from '../configSchema';
...
function validationTests() {
  ...
  it('should throw if collection field pattern is not an array of [string|regex, string]', () => {
    expect(() => {
      validateConfig(merge({}, validConfig, { collections: [{ fields: [{ pattern: [1, ''] }] }] }));
    }).toThrowError(
...
function nestedValidationTests() {
