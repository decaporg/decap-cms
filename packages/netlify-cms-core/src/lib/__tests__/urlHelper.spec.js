...
describe('sanitizeURI', () => {
  ...
  it('should keep valid URI chars (letters digits _ - . ~)', () => {
    expect(sanitizeURI('This, that-one_or.the~other 123!')).toEqual(
...
describe('sanitizeSlug', () => {
  ...
  it('throws an error for non-string replacements', () => {
    expect(() =>
      sanitizeSlug('test', Map({ sanitize_replacement: {} })),
    ).toThrowError('`options.replacement` must be a string.');
    expect(() =>
      sanitizeSlug('test', Map({ sanitize_replacement: [] })),
    ).toThrowError('`options.replacement` must be a string.');
    expect(() =>
      sanitizeSlug('test', Map({ sanitize_replacement: false })),
    ).toThrowError('`options.replacement` must be a string.');
    expect(() =>
      sanitizeSlug('test', Map({ sanitize_replacement: null })),
    ).toThrowError('`options.replacement` must be a string.');
    expect(() =>
      sanitizeSlug('test', Map({ sanitize_replacement: 11232 })),
    ).toThrowError('`options.replacement` must be a string.');
  ...
  it('should remove accents with `clean_accents` set', () => {
    expect(sanitizeSlug('ěščřžý', Map({ clean_accents: true }))).toEqual(
      'escrzy',
    );
  });
  ...
  it('should clean accents and strip non-latin chars in "ascii" mode with `clean_accents` set', () => {
    expect(
      sanitizeSlug(
        'ěščřžý日本語のタイトル',
        Map({ encoding: 'ascii', clean_accents: true }),
      ),
    ).toEqual('escrzy');
  });
  ...
  it('uses alternate replacements', () => {
    expect(
      sanitizeSlug('test   test   ', Map({ sanitize_replacement: '_' })),
    ).toEqual('test_test');
  });
});
