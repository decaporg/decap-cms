...
describe('RichtextPreview Preview renderer', () => {
  describe('RichtextPreview rendering', () => {
    describe('General', () => {
...
`;
        ...
        const { container } = render(
          <RichtextPreview value={html} getAsset={jest.fn()} resolveWidget={jest.fn()} />,
        );
    ...
    describe('Headings', () => {
      for (const heading of [...Array(6).keys()]) {
        it(`should render Heading ${heading + 1}`, async () => {
          ...
          render(<RichtextPreview value={html} getAsset={jest.fn()} resolveWidget={jest.fn()} />);
