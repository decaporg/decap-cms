...
function makeApi(turboFetch: jest.Mock, branch = 'main') {
  return new TurboAPI({
...
describe('TurboAPI.persistFiles', () => {
  ...
  it('commits everything in a single request, with base64 contents', async () => {
    ...
    expect(JSON.parse(init.body)).toEqual({
      branch: 'main',
      // The branch this deploy edits, which the server cannot infer from the
      // sites row — one site serves several deploys of the same repo. A
      // workflow entry forks from it and opens its pull request against it.
      base: 'main',
  ...
  it('bases a workflow save on the branch this deploy edits, not the site row', async () => {
    // The staging case: one Turbo site, two deploys of the same repo, this one
    // editing `develop`. The server cannot know that — sites.branch says
    // `main` — so it travels with the save. Without it the draft forks from
    // production and opens its pull request there.
    const turboFetch = workflowCommitted();
    await makeApi(turboFetch, 'develop').persistFiles(
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      [dataFile] as any,
      [] as any,
      { ...options, useWorkflow: true, collectionName: 'posts', status: 'draft' } as any,
