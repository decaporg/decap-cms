...
**Note:** Folder collections must have at least one field with the name `title` for creating new entry slugs. That field should use the default `string` widget. The `label` for the field can be any string value. See the [collections](https://www.netlifycms.org/docs/add-to-your-site/#collections) section for details on how collections and fields are configured. 
