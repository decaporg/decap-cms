...
5. Open <http://localhost:8000/admin> to verify that your can administer your content locally. (Note: if you get a "connection refused" error, this means that your development server from Step 4 is running on a port that is different from 8000. You can either change the port in this url to match or restart your development server with a different port.)
