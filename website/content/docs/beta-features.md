...
**Note:** Supported events are `prePublish`, `postPublish`, `preUnpublish`, `postUnpublish`, `preSave`, `postSave`. The `preSave` hook can be used to modify the entry data like so:
```
CMS.registerEventListener({
  name: 'preSave',
  handler: ({ entry }) => {
